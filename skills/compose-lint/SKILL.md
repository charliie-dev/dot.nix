...
> Security fundamentals (rootless, cap_drop, read_only, no-new-privileges, socket-proxy, named volumes, init, resource limits) → `container-security` skill.
> If a `Dockerfile` is found in the same directory, also apply the `dockerfile-builder` skill to review how the image is built.
...
2. Check deprecated/renamed options for that version — via context7 MCP (`resolve-library-id` → `query-docs`) when available, otherwise WebSearch or the service's official docs.
